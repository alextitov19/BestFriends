        ...
        VStack {
                ...
                HStack {
                    ...
                    ZStack {
                        ...
                        .frame(width: 75, height: 75, alignment: .center)
                        .background(mood == 8 ? Color.orange : ColorManager .grey1)
                        .cornerRadius(40)
                        .shadow(color: Color(#colorLiteral(red: 0.2067186236, green: 0.2054963708, blue: 0.2076624334, alpha: 1)), radius: 2, x: 0, y: 2)
...
//                                        .frame(height: 30)
        ...
        func shareButtonTapped() {
            ...
            for id in selectedFriends {
                for f in friends {
                    if f.id == id {
                        RestApi.instance.sendPushNotification(title: "BestFriends", body: " \(user.firstName) vibe just changed! Check their 'Atmosphere-color' to see what's up", APNToken: f.APNToken)
                        
                        //MARK: The code below creates an in-app notification for your friend (f.id)
                        //MARK: DO NOT CHANGE THE TEXT OF THE NOTIFICATION, otherwise the code to take the user to a diffrent page will not work. Once you set it, do not change it.
                        RestApi.instance.createInAppNotification(ian: InAppNotification(user: f.id, sender: user.id, text: "vibe just changed! Check their 'Atmosphere-color' to see what's up", createdOn: Int64(Date().timeIntervalSince1970))).then({ response in
