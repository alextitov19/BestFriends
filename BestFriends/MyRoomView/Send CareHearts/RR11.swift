...
//        let arr = [user.id, friend.id]
        ...
        customMessage = "Love CareHeart from; " + user.firstName + " : " + customMessage;
...
//    **********************************
                    ...
                    VStack {
                        ...
                        Button(action: {
                            ...
                            Image("iconShare")
                                ...
                                .alert("Your CareHeart is on it's way", isPresented: $showingAlert) {
    ...
    func shareButtonTapped() {
        ...
        for id in selectedFriends {
            for f in friends {
                if f.id == id {
                    RestApi.instance.sendPushNotification(title: "BestFriends", body: "  \(user.firstName) just send you a CareHeart", APNToken: f.APNToken)
                    
                    //MARK: The code below creates an in-app notification for your friend (f.id)
                    //MARK: DO NOT CHANGE THE TEXT OF THE NOTIFICATION, otherwise the code to take the user to a diffrent page will not work. Once you set it, do not change it.
                    RestApi.instance.createInAppNotification(ian: InAppNotification(user: f.id, sender: user.id, text: "just sent you a CareHeart", createdOn: Int64(Date().timeIntervalSince1970))).then({ response in
