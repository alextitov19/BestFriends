        ...
        for id in selectedFriends {
            for f in friends {
                if f.id == id {
                    ...
                    RestApi.instance.createInAppNotification(ian: InAppNotification(user: f.id, sender: user.id, text: "is blowing you Kisses. Jump up high and catch them.", createdOn: Int64(Date().timeIntervalSince1970))).then({ response in
