                                    ...
                                    ZStack {
                                        ...
                                        Button(action: {
                                                if user.unlockedStickers.contains(number) == false && user.tokens >= 7 { // If user HAS NOT UNLOCKED this sticker but HAS ENOUGH tokens to unlock it (user buys sticker)
                                                    ...
                                                    let message = Message(id: Helper().randomString(length: 20), senderName: user.firstName, senderID: user.id, body: "*Sticker*", creationDate: Int(NSDate().timeIntervalSince1970), stickerNumber: number, hasBeenLiked: false, hasBeenThumb: false)
